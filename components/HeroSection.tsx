  ...
  useEffect(() => {
    async function fetchNews() {
      try {
        ...
        const res = await fetch("/api/berita?potensi=Negatif&pageSize=10&sortField=date&sortOrder=desc");
        if (res.ok) {
      ...
      <div className="relative z-10 max-w-7xl mx-auto px-6 lg:px-12 py-16 lg:py-24 flex flex-col lg:flex-row items-center gap-16">
        ...
        <div className="flex-1 text-center lg:text-left space-y-8">
          ...
          <p className="text-lg md:text-xl text-slate-600 dark:text-blue-100/70 leading-relaxed font-medium max-w-2xl mx-auto lg:mx-0">
            Katakita membantu Anda melihat keterkaitan antar tokoh, isu, dan peristiwa melalui analisis berita media cetak berbasis data dan visualisasi interaktif.
          </p>
        </div>
        ...
        <div className="flex-1 w-full relative mt-8 lg:mt-0">
          ...
          >
            {loading ? (
              ...
              <div className="text-slate-500 dark:text-slate-400 text-center text-sm font-medium">Belum ada berita negatif.</div>
            ) : (
              news.map((item, index) => {
                ...
                return (
                  ...
                  >
                    ...
                    <div className="relative z-10 flex items-center gap-2 mb-4">
                       ...
                       <span className="text-[10px] font-black text-rose-600 dark:text-rose-400 tracking-widest uppercase">Negatif</span>
                       <span className="text-[10px] font-bold text-slate-500 dark:text-slate-400 ml-auto">{item.tanggal}</span>
                    </div>
                    ...
                    <div className="relative z-10 flex items-center gap-2 text-[10px] font-black text-blue-700 dark:text-blue-300 tracking-wider bg-slate-100 dark:bg-slate-900/50 px-3 py-2 rounded-xl border border-slate-200 dark:border-slate-700">
                      <span className="truncate">{Array.isArray(item.pejabat) ? item.pejabat[0] : item.pejabat}</span>
