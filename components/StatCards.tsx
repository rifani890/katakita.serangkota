...
function StatCard({ title, count, icon, colorClass, onClick }: StatCardProps) {
  ...
  return (
    ...
    >
      ...
      <div
        className={`absolute inset-0 bg-gradient-to-br ${theme.gradient} opacity-0 group-hover:opacity-[0.03] dark:group-hover:opacity-10 transition-opacity duration-500 pointer-events-none`}
      />
      ...
      <div className="relative z-10 flex items-center gap-4 sm:gap-5">
        {/* Icon */}
        <div
          className={`w-12 h-12 sm:w-14 sm:h-14 shrink-0 rounded-2xl bg-gradient-to-br ${theme.gradient} flex items-center justify-center text-white shadow-lg ${theme.glow} transition-transform duration-500 group-hover:scale-110 group-hover:rotate-3`}
        >
        ...
        <div className="flex-1 min-w-0">
          <p
            className={`text-[10px] sm:text-xs font-black uppercase tracking-[0.15em] sm:tracking-[0.2em] mb-1 truncate ${theme.text}`}
          >
            {title}
          </p>
        ...
        {/* Arrow indicator */}
        <div
          className={`hidden sm:flex w-8 h-8 rounded-full bg-slate-100 dark:bg-slate-800/50 items-center justify-center opacity-0 group-hover:opacity-100 transform -translate-x-2 transition-all duration-500`}
        >
      ...
      <div
        className={`absolute -bottom-8 -right-8 w-32 h-32 bg-gradient-to-br ${theme.gradient} blur-[50px] opacity-[0.08] dark:opacity-[0.15] group-hover:opacity-[0.2] dark:group-hover:opacity-[0.3] transition-opacity duration-700 rounded-full pointer-events-none`}
      />
    </div>
  );
}
