...
export default function NewsModal({ newsKey, onClose, onPrint, canDelete = false, onDelete }: NewsModalProps) {
        ...
        <div className="p-5 sm:p-8 overflow-y-auto">
          {loading ? (
            <div className="flex flex-col items-center justify-center py-20 gap-4">
              ...
              <p className="text-slate-400 animate-pulse font-medium">Mengambil isi berita...</p>
            </div>
          ) : !news ? (
            ...
            <div className="space-y-6">
              <div className="space-y-2">
                <div className="flex flex-wrap items-center gap-3 text-xs md:text-sm font-semibold text-blue-600 dark:text-blue-400 uppercase tracking-widest">
              ...
              <div className="flex flex-wrap gap-2 py-1">
                {pejabatArray.length > 0 && (
                  <span className="px-2.5 py-0.5 rounded-full text-xs font-semibold uppercase tracking-wider bg-slate-200 dark:bg-slate-700 text-slate-700 dark:text-slate-300">
                    {pejabatArray.join(", ")}
                  </span>
                )}
                {news.unit && (
                  <span className="px-2.5 py-0.5 rounded-full text-xs font-semibold uppercase tracking-wider bg-blue-100 dark:bg-blue-900/30 text-blue-600 dark:text-blue-400">
                    {news.unit}
                  </span>
                )}
                <span
                  className={`px-2.5 py-0.5 rounded-full text-xs font-semibold uppercase tracking-wider ${getSentimenClass(news.potensi)}`}
                >
                  {news.potensi}
                </span>
              </div>
              ...
              <div
                className="text-sm sm:text-base text-justify leading-relaxed text-slate-700 dark:text-slate-300 max-w-none space-y-4 whitespace-pre-wrap"
