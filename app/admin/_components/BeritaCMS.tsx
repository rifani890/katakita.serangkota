...
export default function BeritaCMS({ beritaId }: AdminBeritaCMSClientProps) {
  ...
  const [formTokoh, setFormTokoh] = useState<string[]>([]);
  ...
  const [showTokohDrop, setShowTokohDrop] = useState(false);
  ...
  const tokohDropRef = useRef<HTMLDivElement>(null);
  ...
  useEffect(() => {
    const handler = (e: MouseEvent) => {
      ...
      if (tokohDropRef.current && !tokohDropRef.current.contains(e.target as Node)) {
        setShowTokohDrop(false);
      }
  ...
  const refreshTokoh = useCallback(async () => {
    const res = await fetchWithAuth("/api/tokoh", { cache: "no-store" });
    if (res.ok) {
      const tokohList: TokohItem[] = await res.json();
  ...
  const loadData = useCallback(async () => {
    ...
    try {
      ...
      if (beritaId && newsRes) {
        ...
        setFormTokoh((current) => (current.length > 0 ? current : []));
  ...
  const allMediaNames = Array.from(
    new Set([formMedia, ...allMedia.map((m) => m.nama || m.name || "").filter(Boolean)])
  )
    .filter(Boolean)
  ...
  const filteredTokoh = allTokoh.filter((o) => {
    const name = o.nama || "";
  ...
  const handleQuickAddTokoh = () => {
    ...
    askConfirm("Tambah Tokoh", `Yakin ingin menambahkan tokoh "${name}"?`, "info", async () => {
      ...
      try {
        ...
        if (!res.ok) {
          const data = await res.json().catch(() => ({}));
          throw new Error(data.error || "Gagal menambahkan tokoh");
        }
        setNewTokohName("");
        setNewTokohJabatan("");
        setShowTokohModal(false);
        await refreshTokoh();
        setFormTokoh((prev) => (prev.includes(name) ? prev : [...prev, name]));
      } catch (err) {
        alert((err as Error).message);
      } finally {
        setSaving(false);
  ...
  const handleSave = () => {
    ...
    if (formPejabat.length === 0) {
      alert("Harap pilih setidaknya satu pejabat.");
      return;
    }
  ...
  return (
    <div className="min-h-full bg-slate-100 dark:bg-slate-900">
                  ...
                  <div className="max-h-48 overflow-y-auto p-2 space-y-1">
                    ...
                    {[
                      ...
                      <div className="p-3 text-center text-slate-400 text-xs italic">
                        {newPejabatJabatan.trim()
                          ? `Gunakan "${newPejabatJabatan.trim()}"`
                          : "Ketik untuk mencari/membuat jabatan"}
                      </div>
                    )}
                  </div>
                </div>
      ...
      <Modal
        show={showTokohModal}
        onClose={() => setShowTokohModal(false)}
        title="Tambah Tokoh Manual"
        ...
        <div className="p-6 space-y-4">
          <div className="space-y-2">
            ...
            <input
              type="text"
              value={newTokohName}
              onChange={(e) => setNewTokohName(e.target.value)}
          ...
          <div className="space-y-2">
            ...
            <input
              type="text"
              value={newTokohJabatan}
              onChange={(e) => setNewTokohJabatan(e.target.value)}
              placeholder="Masukkan jabatan atau keterangan tokoh..."
              className="w-full px-4 py-3 rounded-xl bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-700 outline-none focus:ring-2 ring-indigo-500 text-slate-800 dark:text-white"
            />
          </div>
          <button
            onClick={handleQuickAddTokoh}
            disabled={!newTokohName.trim()}
            className="w-full py-4 rounded-xl bg-orange-600 hover:bg-orange-700 text-white font-bold transition-all shadow-lg shadow-orange-500/30 disabled:opacity-50"
          >
            Tambah & Pilih Tokoh
          </button>
        </div>
          ...
          <aside className="order-1 xl:order-2 xl:sticky xl:top-24 flex flex-col gap-5">
            ...
            <section className="order-1 rounded-lg border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800 shadow-sm overflow-visible">
              ...
              <div className="p-5 space-y-5">
                  ...
                  <div className="relative">
                    ...
                    >
                      {!showPejabatDrop ? (
                        <span
                          className={
                            formPejabat.length === 0
                              ? "text-slate-400 text-sm"
                              : "text-slate-800 dark:text-white text-sm font-medium truncate"
                          }
                        >
                          {formPejabat.length === 0
                            ? "Pilih Pejabat"
                            : formPejabat.length === 1
                              ? formPejabat[0]
                              : `${formPejabat.length} Pejabat Terpilih`}
                        </span>
                      ) : (
                        <input
                          autoFocus
                              ...
                              return (
                                ...
                                >
                                  <input
                                    type="checkbox"
                                    checked={checked}
                                    onChange={() => togglePejabat(name)}
                                    className="w-4 h-4 rounded border-slate-300 text-indigo-600 focus:ring-indigo-500"
                                  />
                                  <div className="min-w-0">
                                    <div
                                      className={`text-sm font-semibold truncate ${checked ? "text-indigo-600 dark:text-indigo-400" : "text-slate-700 dark:text-slate-200"}`}
                                    >
                                      {name}
                                    </div>
                                    {o.jabatan && (
                                      <div className="text-[10px] text-slate-400 truncate tracking-wider">
                                        {o.jabatan}
                                      </div>
                                    )}
                                  </div>
                ...
                <div ref={tokohDropRef}>
                  <label className="flex items-center gap-2 text-xs font-black text-slate-500 dark:text-slate-400 tracking-widest mb-2">
                    <Users size={14} /> Nama Tokoh
                  </label>
                  <div className="relative">
                    <div
                      onClick={() => {
                        ...
                        setShowTokohDrop(!showTokohDrop);
                        setShowPejabatDrop(false);
                        setShowUnitDrop(false);
                      }}
                      className="w-full bg-white dark:bg-slate-900 border border-slate-300 dark:border-slate-600 rounded-lg px-3 py-2.5 outline-none focus-within:ring-2 ring-indigo-500 transition-all text-left flex items-center justify-between cursor-text"
                    >
                      {!showTokohDrop ? (
                        ...
                        >
                          {formTokoh.length === 0
                            ? "Pilih Tokoh (Opsional)"
                            : formTokoh.length === 1
                              ? formTokoh[0]
                              : `${formTokoh.length} Tokoh Terpilih`}
                        </span>
                      ) : (
                        <input
                          autoFocus
                          type="text"
                          value={tokohSearch}
                          onChange={(e) => setTokohSearch(e.target.value)}
                          placeholder="Ketik untuk mencari tokoh..."
                          className="bg-transparent outline-none w-full text-sm text-slate-800 dark:text-white placeholder:text-slate-400"
                        />
                      )}
                      <ChevronDown size={16} className="text-slate-400 flex-shrink-0 ml-2" />
                      ...
                      <div className="absolute z-50 left-0 right-0 top-full mt-1 bg-white dark:bg-slate-900 rounded-lg shadow-xl border border-slate-200 dark:border-slate-700 overflow-hidden">
                        <div className="max-h-60 overflow-y-auto p-2 space-y-1">
                          {filteredTokoh.length === 0 ? (
                            ...
                            filteredTokoh.map((o) => {
                              const name = o.nama || "";
                              if (!name) return null;
                              const checked = formTokoh.includes(name);
                              return (
                                <label
                                  key={o.id || name}
                                  className="flex items-center gap-3 p-2.5 hover:bg-slate-50 dark:hover:bg-slate-800 rounded-lg cursor-pointer transition-colors group"
                        ...
                        <div className="p-2 border-t border-slate-100 dark:border-slate-800 bg-slate-50/50 dark:bg-slate-800/50">
                          <button
                            type="button"
                            onClick={() => {
                              setShowTokohModal(true);
                              setShowTokohDrop(false);
                            }}
                            className="w-full flex items-center justify-center gap-2 py-2 text-xs font-bold text-orange-600 hover:bg-orange-50 dark:hover:bg-orange-900/20 rounded-lg transition-all"
                          >
                            <Plus size={14} /> Tambah Tokoh Manual
                          </button>
                        </div>
                      </div>
                    )}
                      ...
                      <div className="absolute z-50 left-0 right-0 top-full mt-1 bg-white dark:bg-slate-900 rounded-lg shadow-xl border border-slate-200 dark:border-slate-700 overflow-hidden">
                        <div className="max-h-60 overflow-y-auto p-2 space-y-1">
                          <button
                            ...
                            className="w-full text-left px-3 py-2.5 text-xs text-slate-500 hover:bg-slate-100 dark:hover:bg-slate-800 rounded-lg transition-colors italic"
                          >
                            -- Kosongkan Pilihan --
                          </button>
