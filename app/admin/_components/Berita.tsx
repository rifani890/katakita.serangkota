...
export default function Berita() {
  ...
  const handleDelete = (key: string, judul: string) => {
    askConfirm(
      "Hapus Berita",
      `Apakah Anda yakin ingin menghapus berita "${judul}" secara permanen?`,
      "delete",
      async () => {
    ...
    <div className="p-4 sm:p-8">
            ...
            <div className="p-5 sm:p-6 space-y-4 overflow-y-auto">
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 text-sm">
                ...
                <div>
                  ...
                  <span className="text-slate-800 dark:text-white font-semibold">
                    {formatDate(viewNews.tanggal_raw)}
                  </span>
                </div>
                <div>
                  <span className="text-slate-400 font-medium">Pejabat:</span>{" "}
                  <span className="text-slate-800 dark:text-white font-semibold">
                    {Array.isArray(viewNews.pejabat)
                      ? viewNews.pejabat.join(", ")
                      : viewNews.pejabat || "-"}
                  </span>
                </div>
                <div>
                ...
                <div>
                  ...
                  <span
                    className={`px-2 py-0.5 rounded-full text-xs font-semibold ${potensiColor[viewNews.potensi] || potensiColor.Netral}`}
                  >
                    {viewNews.potensi || "Netral"}
                  </span>
                </div>
                <div>
              ...
              <div>
                ...
                {viewNews.isi ? (
                  <div
                    className="mt-2 text-slate-700 dark:text-slate-200 text-sm leading-relaxed prose prose-sm dark:prose-invert max-w-none"
                    dangerouslySetInnerHTML={{ __html: DOMPurify.sanitize(viewNews.isi) }}
                  />
                ) : (
                  <p className="mt-2 text-slate-700 dark:text-slate-200 text-sm leading-relaxed">
                    Tidak ada konten.
                  </p>
                )}
              </div>
      ...
      <div className="bg-white dark:bg-slate-800 rounded-xl shadow-sm border border-slate-200 dark:border-slate-700 p-4 sm:p-5 space-y-6">
          ...
          <table className="w-full text-left border-collapse">
            <thead className="bg-slate-200 dark:bg-slate-700 text-slate-700 dark:text-slate-200">
              <tr className="border-b border-slate-300 dark:border-slate-600 divide-x divide-slate-300 dark:divide-slate-600">
                <th className="px-4 py-3 text-xs font-bold w-12 text-center">No</th>
                <th
                  className="px-4 py-3 text-xs font-bold w-28 cursor-pointer hover:text-blue-600 transition-colors"
                  onClick={() => handleSort("tanggal_raw")}
                >
                  Tanggal{" "}
                  {sortField === "tanggal_raw" && <span>{sortOrder === "asc" ? "↑" : "↓"}</span>}
                </th>
                <th
                  className="px-4 py-3 text-xs font-bold cursor-pointer hover:text-blue-600 transition-colors"
                ...
                <th className="px-4 py-3 text-xs font-bold ">Pejabat</th>
                <th className="px-4 py-3 text-xs font-bold ">Unit</th>
                <th className="px-4 py-3 text-xs font-bold ">Segment</th>
                <th
                  className="px-4 py-3 text-xs font-bold cursor-pointer hover:text-blue-600 transition-colors"
                  onClick={() => handleSort("potensi")}
                >
                  Potensi{" "}
                  {sortField === "potensi" && <span>{sortOrder === "asc" ? "↑" : "↓"}</span>}
                </th>
                <th className="px-4 py-3 text-xs font-bold ">User</th>
                <th className="px-4 py-3 text-xs font-bold text-center">Aksi</th>
              </tr>
            </thead>
            <tbody className="divide-y divide-slate-200 dark:divide-slate-700">
              {loading ? (
                ...
                response.items.map((news, index) => {
                  const pejabat = Array.isArray(news.pejabat)
                    ? news.pejabat.join(", ")
                    : news.pejabat || "-";
                  const colorClass = potensiColor[news.potensi] || potensiColor.Netral;
                  const displayUser = news.userEmail
                    ? news.userEmail.replace("@gmail.com", "")
                    : "-";
                  return (
                    <tr
                      key={news.key}
                    ...
                    >
                      ...
                      <td className="px-4 py-4 min-w-[220px]">
                        <div className="text-sm font-bold text-slate-800 dark:text-white group-hover:text-blue-600 transition-colors leading-snug">
                          {news.judul || "-"}
                        </div>
                      </td>
                      ...
                      <td className="px-4 py-4 text-xs text-slate-600 dark:text-slate-300 max-w-[180px]">
                        {pejabat}
                      </td>
                      ...
                      <td className="px-4 py-4 text-xs text-slate-500 max-w-[120px] break-all">
                        {displayUser}
                      </td>
                      <td className="px-4 py-4" onClick={(event) => event.stopPropagation()}>
                        <div className="flex justify-center gap-1.5 flex-wrap">
                          <button
                            onClick={() => setViewNews(news)}
                            className="w-8 h-8 flex items-center justify-center rounded-lg bg-slate-50 text-slate-500 hover:bg-slate-100 dark:bg-slate-700/50 dark:text-slate-400 transition-all"
                            title="Lihat"
                          >
                            <Eye size={13} />
                          </button>
                          <button
                            ...
                            className="w-8 h-8 flex items-center justify-center rounded-lg bg-emerald-50 text-emerald-600 hover:bg-emerald-100 dark:bg-emerald-900/30 dark:text-emerald-400 transition-all"
                            title="Cetak"
                          >
                            <Printer size={13} />
                          </button>
                          <button
                            onClick={() =>
                              router.push(`/admin/berita/cms?id=${encodeURIComponent(news.key)}`)
                            }
                          ...
                          <button
                            onClick={() => handleDelete(news.key, news.judul)}
            ...
            response.items.map((news) => {
              ...
              return (
                ...
                >
                  ...
                  <div
                    className={`absolute inset-0 ${cardGradient} opacity-60 group-hover:opacity-100 transition-opacity pointer-events-none`}
                  />

                  {/* Vertical Accent Bar */}
                  <div
                    className={`absolute left-0 top-0 bottom-0 w-[5px] ${border} opacity-80 group-hover:opacity-100 transition-opacity pointer-events-none`}
                  />
                  ...
                  <div className="flex flex-wrap items-center gap-y-2 gap-x-3 relative z-10 pl-3">
                    ...
                    <div className="flex items-center gap-3">
                      ...
                      <span className="text-[10px] font-bold px-2 py-0.5 rounded-md bg-indigo-50 dark:bg-indigo-900/30 text-indigo-600 dark:text-indigo-400 border border-indigo-100 dark:border-indigo-800/50 tracking-wider">
                        {news.media || "Media"}
                      </span>
                    </div>
                  </div>
                  <div className="relative z-10 pl-3">
                    <div className="text-base font-bold text-slate-800 dark:text-white leading-snug group-hover:text-blue-600 transition-colors">
                      {news.judul}
                    </div>
                  </div>
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 text-xs text-slate-500 dark:text-slate-400 relative z-10 pl-3">
                    <div>
                      <strong className="text-slate-700 dark:text-slate-200">Media:</strong>{" "}
                      {news.media || "-"}
                    </div>
                    <div>
                      <strong className="text-slate-700 dark:text-slate-200">Unit:</strong>{" "}
                      {news.unit || "-"}
                    </div>
                    <div className="sm:col-span-2">
                      <strong className="text-slate-700 dark:text-slate-200">Pejabat:</strong>{" "}
                      {pejabat}
                    </div>
                    <div className="sm:col-span-2 break-all">
                      <strong className="text-slate-700 dark:text-slate-200">User:</strong>{" "}
                      {news.userEmail || "-"}
                    </div>
                  </div>
                  <div className="flex flex-wrap gap-2 pt-1 relative z-10 pl-3">
                    <button
                      onClick={() => setViewNews(news)}
                      className="flex-1 min-w-[120px] inline-flex items-center justify-center gap-2 px-3 py-2 rounded-lg bg-slate-100 dark:bg-slate-800 text-slate-700 dark:text-slate-200 text-sm font-semibold"
                    >
                      <Eye size={14} /> Lihat
                    </button>
                    <button
                      onClick={() => handlePrint(news)}
                      className="flex-1 min-w-[120px] inline-flex items-center justify-center gap-2 px-3 py-2 rounded-lg bg-emerald-100 dark:bg-emerald-900/30 text-emerald-700 dark:text-emerald-300 text-sm font-semibold"
                    >
                    ...
                    <button
                      ...
                      className="flex-1 min-w-[120px] inline-flex items-center justify-center gap-2 px-3 py-2 rounded-lg bg-blue-100 dark:bg-blue-900/30 text-blue-700 dark:text-blue-300 text-sm font-semibold"
                    >
                      <Pencil size={14} /> Edit
                    </button>
                    <button
                      onClick={() => handleDelete(news.key, news.judul)}
                      className="flex-1 min-w-[120px] inline-flex items-center justify-center gap-2 px-3 py-2 rounded-lg bg-rose-100 dark:bg-rose-900/30 text-rose-700 dark:text-rose-300 text-sm font-semibold"
                    >
        ...
        <div className="flex flex-col xl:flex-row gap-4 items-start xl:items-center justify-between border-t border-slate-100 dark:border-slate-800 pt-4">
          ...
          <div className="flex items-center gap-3">
            <span className="text-xs font-bold text-slate-400 tracking-wider">Baris:</span>
            <select
              value={perPage}
          ...
          {response.totalPages > 1 && (
            <div className="flex gap-1.5 items-center">
              <button
                ...
                disabled={page === 1}
                className="w-9 h-9 flex items-center justify-center rounded-lg bg-white dark:bg-slate-800 ring-1 ring-slate-200 dark:ring-slate-700 text-slate-500 disabled:opacity-40 hover:bg-slate-50 transition-all"
              >
                <ChevronLeft size={14} />
              </button>
              {pageNumbers.map((value) => (
                <button
                  ...
                  className={`w-9 h-9 flex items-center justify-center rounded-lg text-sm font-bold transition-all ${page === value ? "bg-indigo-600 text-white" : "bg-white dark:bg-slate-800 ring-1 ring-slate-200 dark:ring-slate-700 text-slate-600 dark:text-slate-400 hover:bg-indigo-50"}`}
                >
                  {value}
                </button>
              ))}
