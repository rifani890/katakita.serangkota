...
export default function Berita() {
 ...
 dangerouslySetInnerHTML={{ __html: viewNews.isi }} 
 />
 ) : (
 <p className="mt-2 text-slate-700 dark:text-slate-200 text-sm leading-relaxed">Tidak ada konten.</p>
 )}
 </div>
 </div>
 ...
 <th className="px-4 py-3 text-xs font-bold w-28 cursor-pointer hover:text-blue-600 transition-colors" onClick={() => handleSort("tanggal_raw")}>
 Tanggal {sortField === "tanggal_raw" && <span>{sortOrder === "asc" ? "↑" : "↓"}</span>}
 </th>
 <th className="px-4 py-3 text-xs font-bold cursor-pointer hover:text-blue-600 transition-colors" onClick={() => handleSort("judul")}>
 Judul {sortField === "judul" && <span>{sortOrder === "asc" ? "↑" : "↓"}</span>}
 </th>
 <th className="px-4 py-3 text-xs font-bold w-28 cursor-pointer hover:text-blue-600 transition-colors" onClick={() => handleSort("media")}>
 ...
 <th className="px-4 py-3 text-xs font-bold cursor-pointer hover:text-blue-600 transition-colors" onClick={() => handleSort("potensi")}>
 ...
 <span className={`px-2 py-1 rounded-full text-xs font-semibold ${colorClass}`}>{news.potensi || "Netral"}</span>
 </td>
 ...
 <span className={`px-2 py-1 rounded-full text-[10px] font-bold ${colorClass}`}>{news.potensi || "Netral"}</span>
