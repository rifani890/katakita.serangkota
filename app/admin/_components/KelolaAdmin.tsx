...
export default function KelolaAdmin() {
  ...
  function handleToggleRole(userData: UserData) {
    ...
    askConfirm(
      "Ubah Role",
      `Apakah Anda yakin ingin mengubah role ${userData.email} menjadi ${nextRole}?`,
      "info",
      async () => {
  ...
  return (
    <div className="p-4 sm:p-8 space-y-8">
      ...
      {toast && (
        <div
          className={`fixed top-20 right-4 z-[9999] px-6 py-4 rounded-2xl shadow-2xl text-white font-bold text-sm flex items-center gap-3 animate-slide-in-right ${
            toast.type === "success"
            ...
            <table className="w-full">
              <thead>
                <tr className="bg-slate-50 dark:bg-slate-900/50 border-b border-slate-200 dark:border-slate-700">
                  <th className="px-6 py-4 text-left text-xs font-bold text-slate-500 dark:text-slate-400 tracking-wider">
                    No
                  </th>
                  <th className="px-6 py-4 text-left text-xs font-bold text-slate-500 dark:text-slate-400 tracking-wider">
                    Pengguna
                  ...
                  <th className="px-6 py-4 text-center text-xs font-bold text-slate-500 dark:text-slate-400 tracking-wider">
                    Aksi
                  </th>
                </tr>
              </thead>
                ...
                {filteredUsers.map((u, idx) => (
                  <tr
                    ...
                    className={`hover:bg-slate-50 dark:hover:bg-slate-700/30 transition-colors ${
                      u.id === user?.uid ? "bg-violet-50/50 dark:bg-violet-900/10" : ""
                    }`}
                  >
              ...
              >
                ...
                <div className="flex flex-wrap gap-2 pt-1">
                  <button
                    onClick={() => handleToggleRole(u)}
                    disabled={u.id === user?.uid}
                    className={`flex-1 min-w-[130px] px-3 py-2 rounded-lg text-xs font-bold transition-all ${
                      u.id === user?.uid
                        ? "bg-slate-100 dark:bg-slate-700 text-slate-400 cursor-not-allowed"
                        : u.role === "admin"
                          ? "bg-amber-100 dark:bg-amber-900/30 text-amber-700 dark:text-amber-400 hover:bg-amber-200 dark:hover:bg-amber-900/50"
          ...
          <div className="bg-white dark:bg-slate-800 w-full max-w-md rounded-3xl shadow-2xl border border-slate-200 dark:border-slate-700 overflow-hidden">
            <div className="p-6 border-b border-slate-100 dark:border-slate-700 bg-gradient-to-r from-violet-50 to-purple-50 dark:from-violet-900/20 dark:to-purple-900/20">
              ...
              <p className="text-sm text-slate-500 dark:text-slate-400 mt-1">
                {editTarget
                  ? "Perbarui data pengguna dan hak akses"
                  : "Masukkan email pengguna yang ingin ditambahkan"}
              </p>
            </div>
              ...
              <div>
                ...
                <div className="flex gap-3">
                  <button
                    ...
                    className={`flex-1 px-4 py-3 rounded-xl font-bold text-sm transition-all border ${
                      newRole === "user"
